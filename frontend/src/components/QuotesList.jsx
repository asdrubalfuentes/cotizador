...
export default function QuotesList({ onEdit }){
  ...
  function remove(q){ if(confirm('Marcar como no viewable?')){ /* TODO: API call to mark */ alert('Marcado (no implementado)') } }
          ...
          <div key={q.file} className="accordion-item">
            <h2 className="accordion-header">
              <button className="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target={`#c${idx}`}>
                {q.quoteNumber} — {q.client || ''} — {q.currency || ''} {q.total || ''}
              </button>
            </h2>
            <div id={`c${idx}`} className="accordion-collapse collapse">
              <div className="accordion-body">
                {q.approvedAt ? <div className="alert alert-success">ACEPTADA por {q.approvedBy} el {q.approvedAt}</div> : null}
